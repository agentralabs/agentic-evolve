#!/bin/bash
# AgenticEvolve — one-liner install script
...
#
# What it does:
#   1. Downloads agentic-evolve-mcp binary to ~/.local/bin/
...
# ── Constants ──────────────────────────────────────────────────────────
REPO="agentralabs/agentic-evolve"
BINARY_NAME="agentic-evolve-mcp"
SERVER_KEY="agentic-evolve"
...
MCP_ENTRYPOINT=""
HOST_OS=""
DEFAULT_STORE_PATH="${HOME}/.agentra/evolve/patterns.aevolve"
SERVER_ARGS_JSON="[\"serve\",\"--mode\",\"full\"]"
SERVER_ARGS_TEXT="${SERVER_ARGS_JSON}"
...
download_binary() {
    ...
    local asset_name="agentic-evolve-${version_num}-${platform}.tar.gz"
    ...
    cp "${tmpdir}"/agentic-evolve-*/evolve "${INSTALL_DIR}/evolve" 2>/dev/null || true
    cp "${tmpdir}"/agentic-evolve-*/${BINARY_NAME} "${INSTALL_DIR}/${BINARY_NAME}"
    chmod +x "${INSTALL_DIR}/evolve" 2>/dev/null || true
...
install_from_source() {
    ...
    if [ "$DRY_RUN" = true ]; then
        echo "  [dry-run] Would run: cargo install --git https://github.com/${REPO}.git --locked --force agentic-evolve-mcp"
        return
    fi
    ...
    cargo install --git "https://github.com/${REPO}.git" --locked --force agentic-evolve-mcp
}
...
configure_claude_desktop() {
    local config_file
    case "${HOST_OS}" in
        Darwin|darwin) config_file="$HOME/Library/Application Support/Claude/claude_desktop_config.json" ;;
...
print_post_install_next_steps() {
    ...
    if [ "$PROFILE" = "server" ]; then
        ...
        echo "  3. If artifacts were created on another machine, sync .aevolve files to this server." >&3
        echo "  4. Start MCP with auth, connect clients, then restart clients." >&3
        echo "  5. Optional feedback: open https://github.com/agentralabs/agentic-evolve/issues" >&3
    else
        echo "  2. Restart your MCP client/system so it reloads MCP config." >&3
        echo "  3. ${SERVER_KEY} now auto-detects local .aevolve files at startup when available." >&3
...
main() {
    ...
    finish_progress
    echo "Install complete: AgenticEvolve (${PROFILE})" >&3
    echo "" >&3
